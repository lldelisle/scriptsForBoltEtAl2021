...
fileInv2PFL="E12.5_inv2_PFL_CHIC.txt"
filewtPFL="E12.5_wt_PFL_CHIC.txt"

###Required parameters
coordinatesToPlot=c("chr2:74638000-74783000__chr2:75129600-75677400")#If only one region is provided, this means you want the classical triangle. If you want to see specific interactions between 2 regions, you need to separate them by __
...
operation1="filewtPFL-fileInv2PFL" #To compute a difference put the name of the file as defined above separated by -


### Optional parameters (Uncomment the lines you want)
##Common for all plots
#Output location
outputPath="/scratch/ldelisle/BoltPlot/figure2b_heatmap" #The output file (without extension)
#Output format
...
normBeforeDiffOrl2R=T #If you do not want to normalize the matrix before doing the difference or the l2R (for example when you are using matrices normalized the same way) remove the # at the beginning of the line.

#For the plot of diff
forceRange=c(-5,4) #if you want to saturate the plot or have the same scale for all plots. This forces the values for Blue and Red.

#For the plot of l2r
#forceRangel2R=c(-10,10) #if you want to saturate the plot or have the same scale for all plots. This forces the values for Blue and Red.
